...
const roleText = computed(() => ({ admin: '管理员', member: '成员' }[profile.value.role] || '成员'))
const statusText = computed(() => ({ pending: '审核中', approved: '已入库', rejected: '已拒绝' }[profile.value.status] || '未知状态'))
